import pandas,numpy,math,scipy
import sys
...
storedval=numpy.full((n,n),-2.0) #using the fact that our criteria lies b/w 0 and 1
for i in range(n):
	print(i)
	var1=data[:,i]
	for j in range(n):
		if storedval[j][i]== -2.0:
			var2=data[:,j]
			tmp=pearsonr(var1,var2)
			info=tmp[0]
		else :
			info=storedval[j][i]
		storedval[i][j]=abs(info)
...
for i in range(n):
	...
	w_four_i=choose(w_one[i],4)+choose(w_one[i],3)*(m - w_one[i])+choose(w_one[i],2)*choose((m - w_one[i]),2)
	#we can vary this for testing
	delta[i]=w_one[i]+w_three_i+w_two_i+w_four_i
	#print(i,w_one[i],delta[i])
	val=100*(delta[i]/(1<<omega))
	delta[i]=float(val)
	#print(delta[i])
print("delta done")
#print(w_one)
...
for i in range(0,n-1):
	var1=data[:,i]
	filtervalue=metrics.mutual_info_score(var1,var2) #here we can use any other filter
